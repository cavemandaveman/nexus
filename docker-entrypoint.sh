#!/bin/sh

set -x

if [ "$1" == 'bin/nexus' ]; then
  if [ -f "${NEXUS_DATA}/keystore.jks" ]; then
    mkdir -p "${NEXUS_HOME}/etc/ssl"
    ln -s "${NEXUS_DATA}/keystore.jks" "${NEXUS_HOME}/etc/ssl/keystore.jks"
    sed \
      -e "s|OBF.*|${JKS_PASSWORD}</Set>|g" \
      -i "${NEXUS_HOME}/etc/jetty-https.xml"
    sed \
      -e "s|nexus-args=.*|nexus-args=\${karaf.etc}/jetty.xml,\${karaf.etc}/jetty-http.xml,\${karaf.etc}/jetty-requestlog.xml,\${karaf.etc}/jetty-https.xml,\${karaf.etc}/jetty-http-redirect-to-https.xml|g" \
      -i "${NEXUS_HOME}/etc/org.sonatype.nexus.cfg"
    grep \
      -q "application-port-ssl" "${NEXUS_HOME}/etc/org.sonatype.nexus.cfg" || \
      sed \
        -e "\|application-port|a\application-port-ssl=8443" \
        -i "${NEXUS_HOME}/etc/org.sonatype.nexus.cfg"
  fi
  sed \
    -e "s|-Xms.*|-Xms${JAVA_MIN_MEM}|g" \
    -e "s|-Xmx.*|-Xmx${JAVA_MAX_MEM}|g" \
    -e "s|karaf.home=.*|karaf.home=${NEXUS_HOME}|g" \
    -e "s|karaf.base=.*|karaf.base=${NEXUS_HOME}|g" \
    -e "s|karaf.etc=.*|karaf.etc=${NEXUS_HOME}/etc|g" \
    -e "s|java.util.logging.config.file=.*|java.util.logging.config.file=${NEXUS_HOME}/etc|g" \
    -e "s|karaf.data=.*|karaf.data=${NEXUS_DATA}|g" \
    -e "s|java.io.tmpdir=.*|java.io.tmpdir=${NEXUS_DATA}/tmp|g" \
    -i "${NEXUS_HOME}/bin/nexus.vmoptions"
  mkdir -p "${NEXUS_DATA}"
  chown -R nexus "${NEXUS_DATA}"
  exec su-exec nexus "$@"
fi

exec "$@"
